...
**📅 当前状态**: Phase 5.1.1 完成 - 改进计划解析精度，解决 Str.matched_group 错误，自主 Agent 鲁棒性大幅提升。
...
**Phase 5.1.1 实现总结** ✅:
- ✅ **LLM 驱动解析**: 实现 `/lib/llm_plan_parser.ml` - 使用 LLM 智能解析计划响应，提取结构化工具调用
- ✅ **鲁棒错误处理**: 全面修复 `Str.matched_group` 错误 - 添加 try-catch 到所有正则表达式匹配点
- ✅ **文件路径清理**: 自动移除 Markdown 反引号、引号，规范化为 `/workspace/` 绝对路径
- ✅ **混合解析策略**: LLM 解析失败时自动降级到简单正则解析，确保系统鲁棒性
- ✅ **验证成功**: 自主 Agent 现在能稳定地规划并执行多步骤任务，无致命解析错误
